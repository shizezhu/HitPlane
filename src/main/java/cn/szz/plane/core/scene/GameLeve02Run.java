...
public class GameLeve02Run extends GameRun {
	...
	public GameLeve02Run(List<Player> playerList) {
		super(SceneNameEnum.GAME_LEVEL_02);
		this.bgImage = new GameBg(new Image(ImageEnum.GAME_BG_02, 0, 0, UIConfig.INSTANCE.getWindowWidth(),
				UIConfig.INSTANCE.getWindowHeight()));
		this.bgSound = new DefaultSound(SoundPathEnum.GAME_LEVEL_02_BG, true);
	...
	protected void produceEnemy(int refreshTimes) {
		if (refreshTimes > 0 && refreshTimes % 7000 == 0) {
			enemyList.add(new Enemy(0, RandomUtils.nextInt(1, 6), RandomUtils.nextInt(1, 5)));
		}
