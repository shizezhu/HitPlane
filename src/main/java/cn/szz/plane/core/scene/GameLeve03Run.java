...
public class GameLeve03Run extends GameRun {

	public GameLeve03Run(List<Player> playerList) {
		super(SceneNameEnum.GAME_LEVEL_03);
		this.bgImage = new GameBg(new Image(ImageEnum.GAME_BG_03, 0, 0, UIConfig.INSTANCE.getWindowWidth(),
				UIConfig.INSTANCE.getWindowHeight()));
		this.bgSound = new DefaultSound(SoundPathEnum.GAME_LEVEL_03_BG, true);
	...
	protected void produceEnemy(int refreshTimes) {
		...
		if (refreshTimes <= 2000) {
			...
			Window.INSTANCE.showScene(new GameLeve04Run(playerList), true);
		}
	}
}
