...
public class GameLeve04Run extends GameRun {

	public GameLeve04Run(List<Player> playerList) {
		super(SceneNameEnum.GAME_LEVEL_04);
		this.bgImage = new GameBg(new Image(ImageEnum.GAME_BG_04, 0, 0, UIConfig.INSTANCE.getWindowWidth(),
				UIConfig.INSTANCE.getWindowHeight()));
		this.bgSound = new DefaultSound(SoundPathEnum.GAME_LEVEL_04_BG, true);
	...
	protected void produceEnemy(int refreshTimes) {
		...
		if (refreshTimes <= 2000) {
			...
			Window.INSTANCE.showScene(new GameLeve05Run(playerList), true);
		}
	}
}
